...
// [0]: directional [1]: point [2]:spot light
Light lights[3];
...
void ChangeSize(GLFWwindow* window, int width, int height)
{
	...
	// Need to let WINDOW_WIDTH & HEIGHT be NOT const to change the value
	WINDOW_WIDTH = width;
...
void RenderScene(void) {
	...
	// to vertex shader
	glUniformMatrix4fv(iLocModel, 1, GL_FALSE, mvp);
	...
	mvp[2] = MVP[8];  mvp[6] = MVP[9];   mvp[10] = MVP[10];  mvp[14] = MVP[11];
	mvp[3] = MVP[12]; mvp[7] = MVP[13];  mvp[11] = MVP[14];  mvp[15] = MVP[15];
	glUniformMatrix4fv(iLocView, 1, GL_FALSE, mvp);
	...
	mvp[2] = MVP[8];  mvp[6] = MVP[9];   mvp[10] = MVP[10];  mvp[14] = MVP[11];
	mvp[3] = MVP[12]; mvp[7] = MVP[13];  mvp[11] = MVP[14];  mvp[15] = MVP[15];
	glUniformMatrix4fv(iLocProjection, 1, GL_FALSE, mvp);
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++)
	{
		...
		glUniform1i(iLocShadingType, 1);
		glUniform1i(iLocLightMode, light_mode);
		glUniform1f(iLocShininess, Shininess);
		glUniform3f(iLocLightPosition, lights[light_mode].position[0], lights[light_mode].position[1], lights[light_mode].position[2]);
		glUniform3f(iLocLightDiffuse, lights[light_mode].diffuse[0], lights[light_mode].diffuse[1], lights[light_mode].diffuse[2]);
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case GeoTranslation:
		models[cur_idx].position += Vector3(0, 0, 0.01 * yoffset);
		break;
	case GeoScaling:
		models[cur_idx].scale += Vector3(0, 0, 0.01 * yoffset);
		break;
	case GeoRotation:
		models[cur_idx].rotation += Vector3(0, 0, 0.01 * yoffset);
		break;
	case ViewEye:
		main_camera.position -= Vector3(0, 0, 0.01 * yoffset);
		setViewingMatrix();
		break;
	case ViewCenter:
	...
	case LightEditing:
		if (light_mode == 0) {
			lights[0].diffuse += Vector3(0.1 * yoffset, 0.1 * yoffset, 0.1 * yoffset);
		}
		if (light_mode == 1) {
			lights[1].diffuse += Vector3(0.1 * yoffset, 0.1 * yoffset, 0.1 * yoffset);
		}
		if (light_mode == 2) {
			lights[2].angle += 0.1 * yoffset;
			cout << lights[2].angle << endl;
		}
		break;
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (mouse_pressed == true) {
		...
		case LightEditing:
			if (light_mode == 0) {
				lights[0].position += Vector3(0.01 * (xpos - starting_press_x), -0.01 * (ypos - starting_press_y), 0);
			}
			else if (light_mode == 1) {
				lights[1].position += Vector3(0.01 * (xpos - starting_press_x), -0.01 * (ypos - starting_press_y), 0);
			}
			if (light_mode == 2) {
				lights[2].position += Vector3(0.01 * (xpos - starting_press_x), -0.01 * (ypos - starting_press_y), 0);
			}
			printf("Light pos:(%f,%f,%f)\n", lights[light_mode].position.x, lights[light_mode].position.y, lights[light_mode].position.z);
			break;
		}
	}
...
void initParameter()
{
	...
	// initialize for lights
	lights[0].position = Vector3(1.0f, 1.0f, 1.0f);
	lights[0].diffuse = Vector3(1.0f, 1.0f, 1.0f);
	lights[1].position = Vector3(0.0f, 2.0f, 1.0f);
	lights[1].diffuse = Vector3(1.0f, 1.0f, 1.0f);
	lights[1].direction = Vector3(0.0f, 0.0f, 0.0f);
	lights[2].position = Vector3(0.0f, 0.0f, 2.0f);
	lights[2].diffuse = Vector3(1.0f, 1.0f, 1.0f);
	lights[2].angle = 30;
	light_mode = 0;
	Shininess = 64;
	//
...
int main(int argc, char **argv)
{
	...
	// create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109164501 HW2", NULL, NULL);
	if (window == NULL)
	{
