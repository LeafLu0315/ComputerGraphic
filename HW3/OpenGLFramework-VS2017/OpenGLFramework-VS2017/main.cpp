...
void RenderScene(int per_vertex_or_per_pixel) {
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++)
	{
		...
		cur_min == 0 ? glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST) : glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
...
int main(int argc, char **argv)
{
	...
	// create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109164501 HW3", NULL, NULL);
	if (window == NULL)
	{
